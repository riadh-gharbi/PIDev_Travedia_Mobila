 ...
 * @author user
 */
public class ListRegionForm extends BaseForm {
    Form current;
    public ListRegionForm (Resources res){        
...
//        });
        ...
        ArrayList<Region>List =serviceRegion.getInstance().affichageRegion();
        for (Region reg : List){
      
            String urlImage = "http://127.0.0.1:8000/uploads/brochures/" + reg.getImage();
            Image placeHolder = Image.createImage(120,90);
            EncodedImage enc = EncodedImage.createFromImage(placeHolder,false);
            URLImage urlim = URLImage.createToStorage(enc, urlImage, urlImage,URLImage.RESIZE_SCALE);
            addButton(urlim,reg,res);
        ...
        Button btnEv= new Button ("Ajouter region");
               addStringValue("", btnEv);
               btnEv.addActionListener(e->{
            new AjoutRegion(res).show();
        });
   ...
   private void addTab(Tabs swipe, Image img, Label spacer, String likesStr, String commentsStr, String text) {
    ...
    private void addButton(Image img,Region reg,Resources res){
        ...
        Label nomTxt = new Label("Nom : "+reg.getNom(),"NewsTopLine1");
       ...
       lSupprimer.addPointerPressedListener(l->{
           ...
           if (dig.show("Suppression", "Vous voulez supprimer cette region ?","Annuler","Oui")){
               dig.dispose();
           }
           else {
               dig.dispose();
               if(serviceRegion.getInstance().SupprimerRegion(reg.getId())){
                   new ListRegionForm(res).show();
                   //refreshTheme();
               }     
           }
       ...
       lModifier.addPointerPressedListener(l->{
           new ModifierRegionForm(res,reg).show();});
       
       cnt.add(BorderLayout.CENTER,BoxLayout.encloseY(BoxLayout.encloseX(nomTxt,lModifier,lSupprimer)));
